...
var Foundation = {
	...
	// 表管理
	table: require('./Library/Table.js').module(Mysql),
...
// console.log( Foundation.table.create() );
...
var server = http.createServer(function (req, res) {
	...
	if ( Foundation[askValue[1]] && Foundation[askValue[1]][askValue[2]] ) {
		var result = Foundation[askValue[1]][askValue[2]](url.parse(req.url, true).query);
		res.write(JSON.stringify(result));
		res.end();
	}
	else {
		res.write(JSON.stringify({ status: 404, message:'' }));
		res.end();
 	}
